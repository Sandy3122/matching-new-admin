...
export const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  const filteredOptions = Object.entries(options).filter(([key, label]) => {
    ...
    return (label as string).toLowerCase().includes(searchLower);
  });
  ...
  const getDisplayText = () => {
    ...
    if (value.length === 1) return options[value[0]];
            ...
            filteredOptions.map(([key, label]) => (
              ...
              >
                <div className="flex items-center space-x-3 w-full">
                  ...
                  <span className="text-sm text-gray-900 flex-1">{label as string}</span>
                </div>
              </DropdownMenuCheckboxItem>
            ))
          )}
